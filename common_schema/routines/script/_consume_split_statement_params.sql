...
--   in	error_on_notfound tinyint unsigned,
...
main_body: begin
    ...
    call _expand_statement_variables(id_from, id_to, split_options, @_common_schema_dummy, should_execute_statement);
    -- select GROUP_CONCAT(token order by id separator '') from _sql_tokens where id between id_from AND id_to into split_options;
    if _is_options_format(split_options) then
      -- if user chooses to specify params in options format, then 'table' is mandatory key.
      ...
      begin
        ...
        call _peek_states_list(id_from, id_to, 'alpha|alphanum|quoted identifier,dot,alpha|alphanum|quoted identifier', false, false, true, table_array_id, peek_match_to);
...
--    if error_on_notfound then
--      if split_table_schema is null or split_table_name is null then
--        call _throw_script_error(id_from, 'Expecting table_schema.table_name in split() statement');
--      end if;
--    end if;
end;
//
